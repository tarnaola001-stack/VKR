  ...
  const { isLoading, error, data } = useQuery({
    ...
    queryFn: () =>
      axiosFetch.get(`/gigs/single/${_id}`)
        .then(({ data }) => {
          if (data) {
            // ИСПРАВЛЕНО ДЛЯ ВКР (Пункт 2): Корректное приведение путей к картинкам портфолио для файл-сервера
            ...
            if (data.images && data.cover && !data.images.includes(data.cover)) {
      ...
      {isLoading ? (
        ...
        <div className="container">
          <div className="left">
            ...
            {/* ИСПРАВЛЕНО ДЛЯ ВКР (Пункт 2): Слайдер портфолио теперь выводит реальные прогруженные изображения */}
            <div className="custom-slider">
              {data?.images && data.images.length > 0 ? (
                <img src={data.images[currentSlide]} alt="portfolio example" className="main-slide-img" />
              ) : (
                <div className="no-portfolio-fallback" style={{ width: "100%", height: "400px", backgroundColor: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", color: "gray" }}>
                  Примеры работ отсутствуют
                </div>
              )}
